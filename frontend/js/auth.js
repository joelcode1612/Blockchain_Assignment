...
function goRegStep(step){
  ...
  document.querySelectorAll('#regDots .dot-bar').forEach((dot,i)=>dot.classList.toggle('on',i<step));
}
...
function finishRegister(){
  const name=document.getElementById('regDisplayName').value.trim()||'Mike Johnson';
  if(!selectedRole){alert('Please select a role.');goRegStep(2);return;}
...
function selectWallet(el,ctx){
  el.parentElement.querySelectorAll('.wallet-opt').forEach(w=>w.classList.remove('selected'));
  el.classList.add('selected');
  ...
  status.innerHTML='<div class="status-dot"></div><span>Connecting to '+walletName+'…</span>';
  setTimeout(()=>{
    status.className='status-line connected';
    status.innerHTML='<div class="status-dot"></div><span>Connected · 0x7a83…4F2E</span>';
