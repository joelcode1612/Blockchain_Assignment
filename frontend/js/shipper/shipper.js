...
//       <td>${ag.carrier?.display_name || "Unknown"}</td>
//       <td>${ag.escrow_amount || "0"} ETH</td>
//       <td>${ag.milestones?.filter((m) => m.status === "verified").length || 0}/${ag.milestones?.length || 0}</td>
//       <td>${new Date(ag.deadline).toLocaleString()}</td>
//       <td><span class="pill ${ag.status === "active" ? "lime" : "amber"}"><span class="dot"></span>${ag.status || "Pending"}</span></td>
//     </tr>
//   `,
//         )
//         .join("");
