...
async function loadMilestones() {
  ...
  if (currentAgreementId === null) {
    document.getElementById('milestoneStatusContainer').innerHTML = '<div style="color:var(--text-faint);padding:12px 0;">Select an agreement to view milestones.</div>';
    document.getElementById('dupMilestone').innerHTML = '<option value="">— Select a milestone —</option>';
    return;
  }

  if (!window.contract) {
    document.getElementById('milestoneStatusContainer').innerHTML = '<div style="color:var(--text-faint);padding:12px 0;">⚠️ Connect wallet first.</div>';
    return;
  }
  ...
  try {
    ...
    document.getElementById('milestoneStatusContainer').innerHTML = `<div style="color:var(--text-faint);padding:12px 0;">❌ Error: ${e.message}</div>`;
  }
}
...
function renderMilestoneStatus(milestones) {
  ...
  if (!milestones || milestones.length === 0) {
    container.innerHTML = '<div style="color:var(--text-faint);padding:12px 0;">No milestones defined.</div>';
    return;
  }
...
window.simulateDuplicate = async function () {
  ...
  if (milestone.paid) {
    ...
    try {
      log(`⏳ Attempting duplicate release for milestone ${milestoneId} (already paid)...`);
      ...
      resultDiv.innerHTML = '<div class="banner ok" style="margin:0;"><div class="banner-icon">⚠️</div><div><strong>Unexpected success.</strong> The contract allowed release on an already-paid milestone. This should not happen.</div></div>';
    } catch (e) {
      // Expected revert – check if the revert reason is "Already released"
      if (e.reason && e.reason.includes('Already released')) {
        log('✅ Duplicate guard blocked the attempt (reverted: "Already released")');
        showToast('✅ Guard blocked duplicate payment!', 'success');
        resultDiv.innerHTML = '<div class="banner ok" style="margin:0;"><div class="banner-icon">🛡</div><div><strong>Guard blocked.</strong> Transaction reverted with reason: "Already released". No funds were transferred.</div></div>';
        duplicateAttempts++;
        document.getElementById('duplicateCount').textContent = duplicateAttempts;
        addAttemptLog('Blocked', `Duplicate release blocked — Milestone ${milestoneId} (${milestone.description})`, e.reason);
      } else {
        // Some other error
    ...
    if (milestone.verified) {
      // Confirm with user
      if (!confirm(`This milestone is verified but not yet paid. Do you want to release it now?`)) return;
      try {
        ...
        resultDiv.innerHTML = '<div class="banner ok" style="margin:0;"><div class="banner-icon">✓</div><div><strong>Payment released successfully.</strong> The milestone is now marked as paid.</div></div>';
        addAttemptLog('Released', `Milestone ${milestoneId} (${milestone.description}) released`, tx.hash);
        ...
        addAttemptLog('Error', `Release failed on milestone ${milestoneId}`, e.message);
      }
    } else {
      // Not verified, can't release
      showToast('❌ Milestone is not verified. Cannot release.', 'error');
      resultDiv.innerHTML = '<div class="banner danger" style="margin:0;"><div class="banner-icon">✕</div><div><strong>Cannot release.</strong> Milestone must be verified first.</div></div>';
      addAttemptLog('Blocked', `Attempt to release unverified milestone ${milestoneId}`, 'Not verified');
    }
  }
};
...
function renderAttemptLog() {
  ...
  if (!attemptLog || attemptLog.length === 0) {
    container.innerHTML = '<div style="color:var(--text-faint);font-size:12px;">No attempts yet.</div>';
    return;
  }
  container.innerHTML = attemptLog.map(entry => {
    const icon = entry.type === 'Blocked' ? 'blocked' : (entry.type === 'Released' ? 'ok' : '');
    const iconClass = entry.type === 'Blocked' ? 'blocked' : (entry.type === 'Released' ? 'ok' : '');
    return `
        <div class="log-row">
